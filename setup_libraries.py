...
class LibraryInstaller:
    ...
    def create_env(self, env_name: str, system_site_packages: bool = True) -> bool:
        # 跳过虚拟环境创建，直接返回成功
        print(f"跳过虚拟环境创建，直接安装到系统环境")
        self.env_name = env_name
        self.system_site_packages = system_site_packages
        self.env_path = Path(sys.executable).parent.parent
        return True
        ...
        try:
            ...
            for pkg in packages:
                ...
                if pkg_name in known_packages:
                    apt_packages.append(known_packages[pkg_name])
                else:
                    pip_packages.append(pkg)
            
            # 使用apt安装已知包
            if apt_packages:
                apt_cmd = f"sudo apt update && sudo apt install -y {' '.join(apt_packages)}"
                print(f"Installing packages via apt: {' '.join(apt_packages)}")
                self.run_command_with_output(apt_cmd, self.debug_output_enabled)
            ...
            if pip_packages:
                ...
                pip_cmd = f"pip3 install -r {temp_req_file} --timeout {timeout} --retries 3 --default-timeout {timeout} --break-system-packages"
                
                fastest_mirror = self.get_fastest_mirror()
                pip_cmd += f" -i {fastest_mirror}"
                print(f"Using mirror: {fastest_mirror}")
                
                success = self.run_command_with_output(pip_cmd, self.debug_output_enabled)
                ...
                print(f"All packages were installed via apt")
                return True
    ...
    def install_rpi_ws281x(self, target_folder_path: str = None) -> bool:
        # 如果没有指定目标路径，使用当前工作目录
        base_path = Path(target_folder_path) if target_folder_path else Path.cwd()

        try:
            source_lib_dir = Path.cwd() / "libraries" / "rpi-ws281x-python"
            dest_lib_dir = base_path / "rpi-ws281x-python"
            ...
            if not python_executable:
                print(f"Error: Cannot find Python executable")
                return False
    ...
    def install_mpu6050(self, target_folder_path: str = None) -> bool:
        ...
        try:
            ...
            dest_lib_dir = base_path / "mpu6050"
    ...
    def install_kit_environment(self):
        ...
        if system_platform == 'windows':
            ...
            # Create virtual environment (now skips and installs to system)
            try:
